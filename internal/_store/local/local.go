...
func (ls *LocalStore) SaveFile(name string, filename string, file multipart.File) error {
	err := ls.FolderCreate(name)
	if err != nil {
		return err
	}
	dst, err := os.Create(path.Join("uploads", name, filename))
	if err != nil {
		return err
	}
	defer dst.Close()
